function Chebyshev2(Fs)
    ...
    % cheb2ord function returns the lowest order of n of the Chebyshev Type
    % II filter that loses no more than Rp dB in the passband and has at
    % least Rs dB of attenuation in the stopband. the scalar (or vector) of
    % corresponding cutoff frequencies Wp is also returned
    [n, Wn] = cheb2ord(Wp, Ws, 3, 80);
    % cheby2 function returns the transfer function coefficients of an
    ...
    freqz(Sos, 512, 500);
    % add figure captions
    title(sprintf('n=%d Pole Chebyshev 2 Lowpass Filter', n));
    % save the figure
    saveas(fig, 'Pole Chebyshev 2 Lowpass Filter.png');